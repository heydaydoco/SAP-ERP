...
- `duty-drawback` 🟧 — **관세환급 (간이정액) shipped** (Phase 7 slices 3–4, migrations 0018–0019). The domain's
  FIRST POSTING document — approve posts Dr 관세환급금 미수금 1140 / Cr 관세환급수익 9830; receipt(입금) posts the
  mirror Dr 보통예금 / Cr 1140 (APPROVED → PAID, 1140 nets to 0). 개별환급 (BOM/소요량) = later.
...
it. **개별환급 (BOM/소요량) only**: this is 간이정액 only. **Deferred (drawback, slice 3):** 개별환급 (제조/소요량
전개), **source 라인당 unique 환급 제약** (중복환급 방지 — 분할환급 정책 확정 시; today a source line can be
claimed more than once, no DB/service guard — accepted scope gap), UNI-PASS 연동, web screens.

**Slice 4 (환급금 입금 클리어링, migration 0019)** is approve's MIRROR — the natural next step that CLOSES the
1140 미수금 approve opened. When 관세청 actually pays, **receipt** posts the mirror FI journal **Dr 보통예금
(BANK_CLEARING 1010 — the finance-accounting clearing slice's key, reused) / Cr 관세환급금 미수금 1140**,
flipping **APPROVED → PAID** so 1140 nets to 0 and the cycle completes. Idempotent on the claim
(`posting_key = drawback:<id>:receipt`; a PAID replay posts nothing); **FULL receipt only** (입금액 ≠ 승인액 →
400). A second `POSTS` doc_flow edge fences the receipt journal too (so the claim now carries **two** POSTS
edges — a PAID replay recovers the receipt journal by its posting key, never by edge scan). New columns
(migration 0019, additive): `receipt_date` / `received_amount` / `received_currency` (all nullable) + the
status CHECK extended to include `PAID`. **Deferred (receipt):** 부분입금 · 관세청 체납상계(차액) · 환차손익 ·
은행명세서 자동매칭 · 은행 마스터 · 입금 취소(reset) · 1140 의 관세청 partner 화(recon) — all backlog.
...
- **Receipt (입금) is approve's MIRROR — idempotent + subledger-owned (§5.1/§5.2).** APPROVED → an atomic
  `UPDATE … WHERE status='APPROVED'` to PAID then `JournalService.post(…, { tx })` with `posting_key =
  drawback:<id>:receipt`: **Dr 보통예금 (BANK_CLEARING) / Cr 관세환급금 미수금 (DUTY_DRAWBACK_RECEIVABLE)** for the
  approved total (KRW, two-line, no FX) — closing the receivable so 1140 nets to 0. PAID → replay (posts
  nothing); non-APPROVED → 409 (a CLAIMED claim must be approved first). **FULL receipt only:** a supplied
  `receivedAmount` must equal the approved total (else 400; compared via `Money.fromNumeric`, so the canonical
  `'5000.0000'` is accepted while sub-won precision 400s). The cash account reuses the finance clearing
  slice's `BANK_CLEARING` string key (→ 1010, currency = null) — no new account_determination key. A second
  `POSTS` edge fences the receipt journal too, so the claim then holds **TWO** POSTS edges; a PAID replay
  recovers the receipt journal by its deterministic posting key, NEVER by scanning POSTS edges (which would
  ambiguously match the approve journal).
...
- **Read-only across domains.** export_declaration(_item) / company are READ-ONLY lookups. The journal is the
  ...
  line twice (→ a duplicate refund). The 입금 클리어링 slice (slice 4) did NOT add it; it still awaits the 분할환급 정책 — until then an accepted,
  documented scope gap (no DB UNIQUE on `source_export_declaration_item_ref`, no service pre-check).
...
## Key tables (migration 0019) — purely additive
- `drawback_claim` gains `receipt_date` (date, nullable), `received_amount` (18,4, nullable, ≥0 CHECK
  `drawback_claim_received_amount_nonneg_ck`), and `received_currency` (char(3), nullable) — all stamped on
  `receipt()`; existing rows stay NULL. The status CHECK is extended `CLAIMED/APPROVED` → `CLAIMED/APPROVED/
  PAID`. No new table, FK, GL account, or number range (`BANK_CLEARING`→1010 / `DUTY_DRAWBACK_RECEIVABLE`→1140
  all pre-seeded).
...
- duty-drawback (receipt) → an `SA` journal (JE-<year> range): **Dr 보통예금 (BANK_CLEARING 1010) / Cr 관세환급금
  미수금 1140**, `approved_total` (KRW), via `JournalService.post(…, { tx })` (caller-tx) — closes the
  receivable approve opened (1140 → net 0). Idempotent on the claim (`posting_key = drawback:<id>:receipt`);
  lineage `drawback_claim` —`POSTS`→ journal (the claim then holds TWO POSTS edges). The cash account reuses
  the finance clearing slice's `BANK_CLEARING` key (currency = null) — no new account_determination key.
...
- duty-drawback (approve, receipt) → each rides its journal's outbox event (`finance.journal.posted`,
  reference `trade.drawback_claim:<docNo>`, same tx) — no event of its own yet.

## Permissions
`trade_compliance:export_declaration:{create,accept,read}` ·
`trade_compliance:import_declaration:{create,accept,read}` ·
`trade_compliance:duty_drawback:{create,approve,receipt,read}` (declared on the controllers; ADMIN `*` covers them).
