 ...
 * substitution + `CurrencyService`/`DbCurrencyRegistry` for the document-rate stamp). Cross-domain reads (the
 * shipment, for existence + company check) go through `@Inject(DB)` directly, READ-ONLY.
 */
@Module({
  imports: [
    PlatformModule,
  ...
  providers: [ShipmentService, FreightSettlementService],
  controllers: [ShipmentController, FreightSettlementController],
  exports: [ShipmentService, FreightSettlementService],
})
